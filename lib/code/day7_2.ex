defmodule Advent.Code7_2 do
  ...
  def judge_hand(play) do
    alph = String.graphemes("AKQT98765432")

    best_hand_str =
      alph
      |> Enum.map(fn fig -> get_hand_strength(Regex.replace(~r/J/, play[:hand], fig)) end)
      |> Enum.max()

    Map.merge(play, %{str: max(best_hand_str, get_hand_strength(play[:hand]))})
  end
